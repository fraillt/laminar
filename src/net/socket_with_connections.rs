use crate::{
    config::Config,
    net::{ConnectionManager, ContextWithSender},
};
use crossbeam_channel::{self, unbounded, Receiver, Sender};
use log::error;
use std::{self, fmt::Debug, io::Result, net::SocketAddr, time::Instant};
...
impl<TSocket: DatagramSocket, ConnectionEvent: Debug> ContextWithSender<ConnectionEvent>
...
#[derive(Debug)]
pub struct SocketWithConnections<TSocket: DatagramSocket, TConnectionManager: ConnectionManager> {
    manager: TConnectionManager,
    receive_buffer: Vec<u8>,
    user_event_receiver: Receiver<TConnectionManager::UserEvent>,
    context: SocketEventSenderAndConfig<TSocket, TConnectionManager::ConnectionEvent>,
    connection_event_receiver: Receiver<TConnectionManager::ConnectionEvent>,
    user_event_sender: Sender<TConnectionManager::UserEvent>,
}

impl<TSocket: DatagramSocket, TConnectionManager: ConnectionManager>
    SocketWithConnections<TSocket, TConnectionManager>
{
    /// Creates an instance of `ConnectionManager` by passing a socket and config.
    pub fn new(socket: TSocket, manager: TConnectionManager, config: Config) -> Self {
        let (connection_event_sender, connection_event_receiver) = unbounded();
        let (user_event_sender, user_event_receiver) = unbounded();
        SocketWithConnections {
            manager,
            receive_buffer: vec![0; config.receive_buffer_max_size],
            user_event_receiver,
            context: SocketEventSenderAndConfig::new(config, socket, connection_event_sender),
            user_event_sender,
    ...
    pub fn manual_poll(&mut self, time: Instant) {
        ...
        loop {
            match context.socket.receive_packet(self.receive_buffer.as_mut()) {
                Ok((payload, address)) => manager.process_packet(context, &address, payload),
                Err(e) => {
            ...
            if context.socket.is_blocking_mode() {
                break;
            }
        }
        ...
        while let Ok(event) = self.user_event_receiver.try_recv() {
            manager.process_event(context, event);
        }

        manager.update_connections(context);
    }
    ...
    /// separate thread.
    pub fn event_sender(&self) -> &Sender<TConnectionManager::UserEvent> {
    ...
    /// a separate thread.
    pub fn event_receiver(&self) -> &Receiver<TConnectionManager::ConnectionEvent> {
    ...
    pub fn socket_mut(&mut self) -> &mut TSocket {
        &mut self.context.socket
    }

    /// Returns a connection manager reference.
    #[allow(dead_code)]
    pub fn manager(&self) -> &TConnectionManager {
        &self.manager
    }

    /// Returns a mutable connection manager reference.
    #[allow(dead_code)]
    pub fn manager_mut(&mut self) -> &mut TConnectionManager {
        &mut self.manager
    }
}
