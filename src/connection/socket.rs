use crate::net::SocketWithConditioner;
use crate::net::{DatagramSocket, SocketWithConnections};
...
use super::connection_manager::Manager;
use super::events::{ConnectionEvent, UserEvent};

#[derive(Debug)]
pub struct NewSocket {
    handler: SocketWithConnections<SocketWithConditioner, Manager>,
}
...
impl NewSocket {
    ...
    /// separate thread.
    pub fn get_event_sender(&self) -> Sender<(SocketAddr, UserEvent)> {
    ...
    /// a separate thread.
    pub fn get_event_receiver(&self) -> Receiver<(SocketAddr, ConnectionEvent)> {
        self.handler.event_receiver().clone()
    }

    /// Sends a single user event.
    pub fn send(&mut self, event: (SocketAddr, UserEvent)) -> Result<()> {
        self.handler
            .event_sender()
            .send(event)
    ...
    pub fn recv(&mut self) -> Option<(SocketAddr, ConnectionEvent)> {
